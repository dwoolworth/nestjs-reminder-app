...
export class NoteController {
    ...
    async createNotes(@Req() req: Request, @Param('id') id: string, @Body() createNoteDto: CreateNoteDto) {
    ...
    @ApiResponse({ status: 201, description: 'The note has been successfully updated for the reminder. '})
    @ApiResponse({ status: 400, description: 'Bad Request.' })
    async updateNote(@Req() req: Request, @Param('id') id: string, @Param('noteId') noteId: string, @Body updateNoteDto: UpdateNoteDto): Promise<Note> {
        const userId = this.getUserId(req);
        const reminder =  this.reminderService.findOne(userId, id);
