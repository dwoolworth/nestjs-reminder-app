...
export class NoteService {
  ...
  async find(reminderId: string, noteId: string): Promise<Note> {
    const note = await this.noteModel
      .findOne({ _id: noteId, reminder: reminderId })
      .exec();
  ...
  async remove(reminder: ReminderDocument, noteId: string): Promise<Note> {
    ...
    reminder.notes = reminder.notes.filter(
      (id: ObjectId) => id.toString() === noteId,
    );
    await reminder.save();
